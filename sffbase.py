...
class sffbase(object):
    ...
    def parse_sff_element(self, eeprom_data, eeprom_ele, start_pos):
        ...
        if type == 'enum':
            ...
            # Get the 'on' bitname
            bitvalue_dict = {}
            for bitname, bitinfo in sorted(six.iteritems(decode)):
    ...
    def parse_sff(self, eeprom_map, eeprom_data, start_pos):
        outdict = {}
        for name, meta_data in sorted(six.iteritems(eeprom_map)):
    ...
    def get_data_pretty_dict(self, indict):
        ...
        for elem, elem_val in sorted(six.iteritems(indict)):
